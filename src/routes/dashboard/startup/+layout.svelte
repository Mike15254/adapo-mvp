<script lang="ts">
    ...
    let isLoading = false; // Changed initial state to false
    ...
    async function navigationTo(href: string) {
        ...
        try {
            ...
            isLoading = false; // Ensure loading state is reset after navigation
        }
    }
                        ...
                        <li>
                            <ul role="list" class="-mx-2 space-y-1">
                                {#each navigationItems as item}
                                    <li>
                                        <button
                                            on:click|preventDefault={() => navigationTo(item.href)}
                                    ...
                                    <li>
                                        <ul role="list" class="-mx-2 space-y-1">
                                            {#each navigationItems as item}
                                                <li>
                                                    <button
                                                        on:click|preventDefault={() => { navigationTo(item.href); isMobileMenuOpen = false; }}
                                                        class="group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6"
                                                        class:text-indigo-600={$page.url.pathname === item.href}
                                                        class:bg-indigo-50={$page.url.pathname === item.href}
                                                        class:text-gray-700={$page.url.pathname !== item.href}
