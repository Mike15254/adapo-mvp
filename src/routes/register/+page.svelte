<script lang="ts">
  ...
  import { userType, user } from '$lib/stores/userStore';
  ...
  async function handleSubmit() {
    if (!$userType) {
      error = 'Please select a user type first';
      return;
    }
    ...
    try {
      const userData = await registerUser(email, password, passwordConfirm, name, $userType);
      user.set(userData);
      goto('/dashboard');
    } catch (err) {
